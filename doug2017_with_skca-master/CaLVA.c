...
#define ica _p[9]
#define icaLVA _p[10]
#define dLVAinf _p[11]
#define fLVAinf _p[12]
#define dLVAtau _p[13]
#define fLVAtau _p[14]
#define dLVA _p[15]
#define fLVA _p[16]
#define eca _p[17]
#define DdLVA _p[18]
...
extern void hoc_reg_nmodl_filename(int, const char*);
 ...
 "boltz_caLVA", _hoc_boltz,
 0, 0
};
#define boltz boltz_caLVA
 ...
 "gcaLVAbar_caLVA", "S/cm2",
 "vhalfAct_caLVA", "mV",
 "vhalfInact_caLVA", "mV",
 "vhalfTact_caLVA", "mV",
 "vhalfTInact_caLVA", "mV",
 "ica_caLVA", "mA/cm2",
 "icaLVA_caLVA", "mA/cm2",
 "dLVAinf_caLVA", "1",
 "fLVAinf_caLVA", "1",
 ...
 static double dLVA0 = 0;
 static double fLVA0 = 0;
...
extern void _cvode_abstol( Symbol**, double*, int);
 ...
 _mechtype = nrn_get_mechtype(_mechanism[1]);
  ...
  hoc_register_dparam_semantics(_mechtype, 3, "cvodeieq");
 	...
 	ivoc_help("help ?1 caLVA C:/Users/zprice11/Desktop/doug2017_with_skca-master/CaLVA.mod\n");
 hoc_register_limits(_mechtype, _hoc_parm_limits);
 hoc_register_units(_mechtype, _hoc_parm_units);
 }
static int _reset;
static char *modelname = "LVA calcium current (CaLVA-current) ";
...
/*CVODE*/
 static int _ode_spec1 (double* _p, Datum* _ppvar, Datum* _thread, _NrnThread* _nt) {int _reset = 0; {
   double _ldLVAinf , _ldLVAtau , _lfLVAinf , _lfLVAtau ;
 _ldLVAinf = 1.0 / ( 1.0 + exp ( - ( v - vhalfAct ) / slopeAct ) ) ;
   _lfLVAinf = 1.0 / ( 1.0 + exp ( - ( v - vhalfInact ) / slopeInact ) ) ;
   _ldLVAtau = 4.45 / ( 1.0 + exp ( - ( v - vhalfTact ) / slopeTact ) ) + 0.95 ;
   _lfLVAtau = 5.0 / ( 1.0 + exp ( - ( v - vhalfTInact ) / slopeTInact ) ) + 15.0 ;
   DdLVA = ( _ldLVAinf - dLVA ) / _ldLVAtau ;
   DfLVA = ( _lfLVAinf - fLVA ) / _lfLVAtau ;
   }
 return _reset;
}
 ...
 _ldLVAtau = 4.45 / ( 1.0 + exp ( - ( v - vhalfTact ) / slopeTact ) ) + 0.95 ;
 _lfLVAtau = 5.0 / ( 1.0 + exp ( - ( v - vhalfTInact ) / slopeTInact ) ) + 15.0 ;
 DdLVA = DdLVA  / (1. - dt*( ( ( ( - 1.0 ) ) ) / _ldLVAtau )) ;
 DfLVA = DfLVA  / (1. - dt*( ( ( ( - 1.0 ) ) ) / _lfLVAtau )) ;
  return 0;
}
 /*END CVODE*/
 ...
 _ldLVAinf = 1.0 / ( 1.0 + exp ( - ( v - vhalfAct ) / slopeAct ) ) ;
   ...
   _ldLVAtau = 4.45 / ( 1.0 + exp ( - ( v - vhalfTact ) / slopeTact ) ) + 0.95 ;
   _lfLVAtau = 5.0 / ( 1.0 + exp ( - ( v - vhalfTInact ) / slopeTInact ) ) + 15.0 ;
    dLVA = dLVA + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / _ldLVAtau)))*(- ( ( ( _ldLVAinf ) ) / _ldLVAtau ) / ( ( ( ( - 1.0 ) ) ) / _ldLVAtau ) - dLVA) ;
    fLVA = fLVA + (1. - exp(dt*(( ( ( - 1.0 ) ) ) / _lfLVAtau)))*(- ( ( ( _lfLVAinf ) ) / _lfLVAtau ) / ( ( ( ( - 1.0 ) ) ) / _lfLVAtau ) - fLVA) ;
   }
  return 0;
}
...
static void _initlists(){
 ...
 int _i; static int _first = 1;
  if (!_first) return;
 _slist1[0] = &(dLVA) - _p;  _dlist1[0] = &(DdLVA) - _p;
 _slist1[1] = &(fLVA) - _p;  _dlist1[1] = &(DfLVA) - _p;
_first = 0;
}
